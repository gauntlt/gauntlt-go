...
func (c *Context) RunDir(dir string) (*Runner, error) {
	g, _ := filepath.Glob(filepath.Join(dir, "*.attack"))
	g2, _ := filepath.Glob(filepath.Join(dir, "**", "*.attack"))
	g = append(g, g2...)
...
func (c *Runner) runFeature(f *gherkin.Attack) {
...
func (c *Runner) runScenario(title string, f *gherkin.Attack, s *gherkin.Scenario, isExample bool) {
	if !s.FilterMatched(f, c.Filters...) {
		return
	}
