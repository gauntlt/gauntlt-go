...
func ParseFilename(data, filename string) ([]Attack, error) {
	...
	if len(p.attacks) == 0 {
		return p.attacks, p.err("no attacks were parsed")
	}
...
func (p *parser) parse() error {
	for p.stillReading() {
		if err := p.consumeAttack(); err != nil {
			return err
		}
	}
...
func (p *parser) consumeAttack() error {
	desc := []string{}
	...
	if !p.nextLine() {
		if len(tags) > 0 {
			return p.err("tags not applied to attack")
		}
		return nil
	}
	...
	for p.stillReading() { // consume until we get to Background or Scenario
		...
		if p.nextLine() {
			...
			if indent <= startIndent { // de-dented
				break // done parsing this attack
			}
		}
